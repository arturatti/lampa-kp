(function() {
    ...
    function processKinopoiskData(data) {
        ...
        if (data && data.data.userProfile && data.data.userProfile.userData && data.data.userProfile.userData.plannedToWatch) {
            ...
            console.log('Kinopoisk', 'Final kinopoisk_movies saved:', kinopoiskMovies);
            ...
            receivedMovies.forEach(m => {
                ...
                if (!existsInLocalStorage) {
                    ...
                    var kinopoiskId = String(m.movie.id);
                    network.silent('https://kinopoiskapiunofficial.tech/api/v2.2/films/' + kinopoiskId, function(kpData) {
                        var movieIMDBid = kpData.imdbId;
                        var movieTitle = kpData.nameOriginal || kpData.nameRu || title;
                        var movieType = kpData.type || m.movie.type || 'FILM';
                        var movieYear = kpData.year || m.movie.year || '';

                        var tmdbApiKey = '4ef0d7355d9ffb5151e987764708ce96';
                        var tmdbUrl = '';

                        if (movieIMDBid) {
                            console.log('Kinopoisk', 'IMDB id found: ' + movieIMDBid + ' for ' + kinopoiskId);
                            tmdbUrl = 'https://api.themoviedb.org/3/find/' + movieIMDBid + '?external_source=imdb_id&language=ru&api_key=' + tmdbApiKey;
                        } else {
                            console.log('Kinopoisk', 'No IMDB id, fallback to search: ' + movieTitle + ', ' + movieYear);
                            if (movieType === 'FILM') {
                                tmdbUrl = 'https://api.themoviedb.org/3/search/movie?query=' + encodeURIComponent(movieTitle) + '&year=' + movieYear + '&language=ru&api_key=' + tmdbApiKey;
                            } else {
                                tmdbUrl = 'https://api.themoviedb.org/3/search/tv?query=' + encodeURIComponent(movieTitle) + '&year=' + movieYear + '&language=ru&api_key=' + tmdbApiKey;
                            }
                        }

                        var proxyUrl = 'https://script.google.com/macros/s/AKfycbwmrtqfj-rHs0obsb0VsPilHBdeJba45pVcE4VnOF43EzTYi2RUDZWpl9wlBkn1bez9/exec?url=' + encodeURIComponent(tmdbUrl);

                        network.silent(proxyUrl, function(data) {
                            var movieItem = (data.results && data.results[0]) || (data.movie_results && data.movie_results[0]) || (data.tv_results && data.tv_results[0]);
                            if (movieItem) {
                                movieItem.kinopoisk_id = kinopoiskId;
                                ...
                                if (movieDate <= new Date()) {
                                    ...
                                    if (Lampa.Storage.get('kinopoisk_add_to_favorites', false)) {
                                        Lampa.Favorite.add('wath', movieItem, 100);
                                    }
                                }
                            } else {
                                console.log('Kinopoisk', 'No match found in TMDB for: ' + movieTitle + ', ' + movieYear);
                            }
                            calculateProgress(receivedMoviesCount, processedItems++);
                        }, function(err) {
                            console.log('Kinopoisk', 'TMDB fetch error: ', err);
                            calculateProgress(receivedMoviesCount, processedItems++);
                        });

                    }, function(err) {
                        console.log('Kinopoisk', 'kinopoiskapiunofficial error: ', err);
                        calculateProgress(receivedMoviesCount, processedItems++);
                    }, false, {
                        type: 'get',
                        headers: {
